...
def generate_scenes(gen, modis_vars, modis_mask):
    ...
    # recon_image = scene_gen
    return recon_image


def plot_samples_cmp(gen, scene_real, modis_vars, modis_mask):
    ...
    np.savez(dir_path+"/../data/scenes_output.npz", scene_initial=scene_initial, scene_real=scene_real, scene_gen=scene_gen)
...
def plot_all(scenes_fn, model_name="cs_modis_cgan-release"):
    ...
    (wind_scenes, modis_vars, modis_mask) = scenes["validate"]

    # plot_samples_cmp(gen, scene_real, modis_vars, modis_mask)
    plot_samples_cmp(gen, wind_scenes, modis_vars, modis_mask)
